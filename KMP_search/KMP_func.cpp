...
void prefix(char *mass, int *prfx, int n) 
{
    ...
    for(int i = 1; i < n; i++){						           // Перебираем элементы массива.
        int n = prfx[i - 1];						           // Индефикатор равен предыдущему элементу массива.

        while(n > 0 && mass[i] != mass[n])			           // Сравниваем элемент исходного массива с префиксами до нее.
            n = prfx[n - 1];

        if(mass[i] == mass[n])						           // Если элементы равны - то увеличиваем индефикатор на 1.
            n++;

        prfx[i] = n;								           // Если этот элемент продолжает следование, то увеличиваем префикс.
    }
}
...
int kmp_search(char *mass, char *templa, int *prfx, int *result, int mass_l, int templa_l)
{
	...
	while (indent < mass_l)										// Пока не будет достинут конец массива строки.
	{
		...
		{
			step = match - prfx[match]-1; // Если участок строки и шаблон не совпали, либо совпали частично. 
			if (step < 0)
				step++;
		}
